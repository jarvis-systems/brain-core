...
class DoBrainstormInclude extends IncludeArchetype
{
    ...
    protected function handle(): void
    {
        ...
        $this->guideline('phase0-context-setup')
            ...
            ->phase(VectorMemoryMcp::callValidatedJson('search_memories', ['query' => '{$BRAINSTORM_TOPIC}', 'limit' => 5]))
            ->phase(Store::as('MEMORY_CONTEXT', '{related past solutions and patterns}'))
            ->phase(VectorMemoryMcp::callValidatedJson('search_memories', ['query' => '{$BRAINSTORM_TOPIC} best practices architecture', 'limit' => 3, 'category' => 'architecture,learning']))
            ...
            ->phase(Operator::if('user requests task creation', [
                ...
                Operator::if('user chooses standalone', [
                    Operator::forEach('item in $ACTIONABLE_ITEMS', [
                        VectorTaskMcp::callValidatedJson('task_create', ['title' => '{item.title}', 'content' => '{item.description}', 'priority' => '{item.priority}', 'estimate' => '{item.estimate}', 'order' => '{item.order}', 'parallel' => '{item.parallel}', 'tags' => [self::TAG_SPIKE]]),
                    ]),
                ...
                Operator::if('user provides parent task ID', [
                    ...
                    Operator::forEach('item in $ACTIONABLE_ITEMS', [
                        VectorTaskMcp::callValidatedJson('task_create', ['title' => '{item.title}', 'content' => '{item.description}', 'parent_id' => '$PARENT_TASK_ID', 'priority' => '{item.priority}', 'estimate' => '{item.estimate}', 'order' => '{item.order}', 'parallel' => '{item.parallel}', 'tags' => [self::TAG_SPIKE]]),
                    ]),
                    Store::as('CREATED_TASK_IDS', '[ids...]'),
        ...
        $this->guideline('phase4-completion')
            ...
            ->phase(VectorMemoryMcp::callValidatedJson('store_memory', ['content' => 'Brainstorm Session: {$BRAINSTORM_TOPIC}\n\nContext:\n{context_summary}\n\nApproaches Discussed:\n{approaches}\n\nKey Insights:\n{insights}\n\nDecisions Made:\n{decisions}\n\nNext Steps:\n{next_steps}\n\nTasks Created: {task_ids or none}', 'category' => self::CAT_ARCHITECTURE, 'tags' => [self::MTAG_DECISION, self::MTAG_REUSABLE]]))
            ->phase(Operator::output([
                '',
                '=== BRAINSTORM COMPLETE ===',
                'Topic: {$BRAINSTORM_TOPIC}',
